...
`proxy: port: "443"` - порт через который устанавливается Vless соединение (либо через env переменную SING_BOX_PORT)
...
```bash
docker run -d -p 8030:8030 \  #тут обычные порты для ручек
  -p 443:443 \  #внутренний порт остается без изменений, а внешний на ваш выбор, но он должен соответствовать значению из конфига
  --env-file .env  \  #если пользуетесь .env
  -v $(pwd)/data:/app/data vlessproxy #опционально. должен соответствовать значениб из конфига
```
